...
define(['jquery', 'core/ajax', 'core/notification', 'core/str', 'core/url', 'block_eduvidual/main'], function($, AJAX, NOTIFICATION, STR, URL, MAIN) {
    return {
        ...
        */
        questionBankCategoryAddFilter: function() {
            var OBSERVER = this;
            OBSERVER.inci++;
            if (OBSERVER.debug) console.log('block_eduvidual/ajax_observer:questionBankCategoryAddFilter(), inci is ', OBSERVER.inci);
            STR.get_strings([
                {'key' : 'questioncategoryfilter:label', component: 'block_eduvidual' },
            ]).done(function(s) {
                var keyup = 'var i = $(\'#id_filtercategory_' + OBSERVER.inci + '\'); require([\'block_eduvidual/main\', \'block_eduvidual/ajax_observer\'], function(M, OBSERVER) { M.watchValue({ target: i, run: function() { OBSERVER.questionBankCategoryFilter($(i)); } }); }); return false;';
    ...
    **/
    confirmed: function(selector, success, timeout) {
        ...
        console.log('block_eduvidual/ajax_observer:confirmed(selector, success, timeout)', selector, success, timeout);
