define(['jquery', 'core/ajax', 'core/notification', 'core/str', 'core/url', 'local_eduvidual/main'], function($, AJAX, NOTIFICATION, STR, URL, MAIN) {
 ...
 uniqid = $('.local_eduvidual_admin_map').attr('id');
}
      if (typeof lon1 === 'undefined') {
 lon1 = -200;
}
      if (typeof lon2 === 'undefined') {
 lon2 = 200;
}
 ...
 console.log('Sending to ' + method, data);
      ...
      AJAX.call([{
        ...
        args: data,
          ...
          if (ADMIN.refresh_identifier != refresh_identifier) {
 return;
} // Immediately abort!
          try {
 ...
 console.log('Setting bounds to', bounds);
    ...
    org_gps_list: function(uniqid) {
      var ADMIN = this;
      if (ADMIN.debug_gps) {
 console.log('ADMIN.org_gps_list(uniqid)', uniqid);
}
      if (typeof ADMIN.map !== 'undefined') {
        ADMIN.map.remove();
      }
 ...
 return;
        ...
        if (col != 'lightgray' && orgtype > 0) {
          if (orgs[i].lon < smallest_lon) {
 smallest_lon = orgs[i].lon;
}
          if (orgs[i].lon > biggest_lon) {
 biggest_lon = orgs[i].lon;
}
          if (orgs[i].lat < smallest_lat) {
 smallest_lat = orgs[i].lat;
}
          if (orgs[i].lat > biggest_lat) {
 biggest_lat = orgs[i].lat;
}
        }
 ...
 return;
      ...
      ADMIN.map.on('resize', function(ev) {
        if (ADMIN.debug_gps) {
 console.log('Map was resized', ev);
}
        var width = $('#' + uniqid + '-map').width();
        if (ADMIN.debug_gps) {
 console.log('Width', width);
        ...
        if (ADMIN.debug_gps) {
 console.log('Height', width * mapRatio);
}
 ...
 return;
        ...
        if (typeof triggerkey !== 'undefined' && triggerkey !== '') {
          ...
          if (typeof filterid !== 'undefined' && filterid != '') {
            if (typeof sums[filterid] === 'undefined') {
 sums[filterid] = 0;
}
    ...
    questioncategories: function(uniqid, catid) {
      if (this.debug > 0) {
 console.log('ADMIN.questioncategories(uniqid, catid)', uniqid, catid);
}
    ...
    requireCapability: function(val) {
      if (this.debug > 0) {
 console.log('ADMIN.requireCapability(val)', val);
}
      require(['local_eduvidual/main'], function(MAIN) {
