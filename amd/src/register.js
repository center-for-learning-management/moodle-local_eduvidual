    ...
    return {
        ...
        result: function(o) {
            if (o.result && o.result.status && o.result.status == 'ok') {
                ...
                if (o.data.stage == 0) {
                    $('#block_eduvidual_registration_stage-1, #block_eduvidual_registration_stage-0-error').css({ display: 'none'});
                    if (o.result.authenticated == 1) {
                        $('#block_eduvidual_registration_stage-0-error').css({ display: 'block'});
                    } else {
                        $('#block_eduvidual_registration_stage-1').css({ display: 'block'});
                    }
                ...
                if (o.data.stage == 1) {
                    STR.get_strings([
                        {'key' : 'mailregister:confirmation', component: 'block_eduvidual' },
                        {'key' : 'mailregister:confirmation:mailsent', component: 'block_eduvidual' },
                        {'key' : 'ok' }
                    ]).done(function(s) {
                ...
                if (o.data.stage == 2) {
                    ...
                    $('#block_eduvidual_registration_stage-0').css({ display: 'none'});
                    $('#block_eduvidual_registration_stage-1').css({ display: 'none'});
                    $('#block_eduvidual_registration_stage-2').css({ display: 'block'});
                    $('#block_eduvidual_registration_homecategory').attr('href', '/course/index.php?categoryid=' + o.result.categoryid).html(o.result.name);
                }
            } else {
                // @todo localization
