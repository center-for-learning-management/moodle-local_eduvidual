...
$PAGE->set_url('/local/eduvidual/pages/login_mnet.php', array());
$PAGE->set_title(get_string('Login', 'local_eduvidual'));
$PAGE->set_heading(get_string('Login', 'local_eduvidual'));
//$PAGE->set_cacheable(false);
$PAGE->requires->css('/local/eduvidual/style/login_mnet.css');
...
$orgs = $DB->get_records_sql('SELECT * FROM {local_eduvidual_org} WHERE mnetid > 1 ORDER BY name ASC', array());
foreach ($orgs AS $org) {
    ...
    foreach($mnets AS $mnet) {
        ...
        if ($org->orgid == 601457 && $mnet->id == 3) {
            $host->logo = $CFG->wwwroot . '/pluginfile.php/1/local_eduvidual/globalfiles/0/_sys/bulme-tweak/bulme.png';
