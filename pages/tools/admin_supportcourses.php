...
foreach ($orgs AS $org) {
    ...
    if (!empty($org->supportcourseid) && !empty($course->id)) {
        ...
        $supportcourse = \block_eduvidual\lib_helper::duplicate_course($template, 'Helpdesk (' . $org->name . ')', 'helpdesk_' . $org->orgid, $org->categoryid, 1);
        if (empty($supportcourse->id)) {
            echo "<li class='alert alert-danger'><strong>Error creating supportcourse</strong></li>\n";
        } else {
            $DB->set_field('block_eduvidual_org', 'supportcourseid', $supportcourse->id, array('orgid' => $org->orgid));
            ...
            $members = $DB->get_records('block_eduvidual_orgid_userid', array('orgid' => $org->orgid));
            $managers = array();
            $others = array();
            ...
            \block_eduvidual\lib_enrol::course_manual_enrolments(array($supportcourse->id), $managers, get_config('block_eduvidual', 'defaultroleteacher'));
            \block_eduvidual\lib_enrol::course_manual_enrolments(array($supportcourse->id), $others, get_config('block_eduvidual', 'defaultrolestudent'));
